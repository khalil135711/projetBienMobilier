...
<head>
    ...
    <script>
        ...
        const SHOW_CLIPBOARD_MONITORING_CONTROLS = true;
...
<body>
    <h1><a href="https://github.com/six-two/qr.html" target="_blank" rel="noopener noreferrer">QR code generator</a></h1>
    ...
    <textarea id="input" name="input" id="" autofocus>Put your text in this box. For example:
https://example.com

- 🎉 This should now also work with Emojis
- ⚠️ The maximum size of 2950 bytes still exists</textarea>
...
<script type="text/javascript">
    ...
    <script>
        // All the clipboard stuff is here in a separate script, so that you can delete it if you don't need it (makes the file smaller)
        if (SHOW_CLIPBOARD_MONITORING_CONTROLS) {
            const clipboard_checkbox = document.getElementById("clipboard-checkbox");
            const clipboard_error = document.getElementById("clipboard-error");

            const showClipboardError = (message) => {
                console.log("Clipboard error:", message);
                clipboard_error.innerHTML = message;
                clipboard_error.style.display = message ? "block" : "none";
            };
            ...
            const check_clipboard = () => {
                if (clipboard_checkbox.checked) {
                    ...
                    try {
                        ...
                        showClipboardError('Failed to read clipboard contents. This is a known problem in Firefox, please try using a different Browser like Chrome, Edge, etc (anything except Internet Explorer).<br><br>If you only have Firefox, you could enable Clipboard access by typing "about:config" in the URL bar, searching for "dom.events.testing.asyncClipboard" and setting it to true (by double clicking). Afterwards you can reload the page. <b>Please note, that doing this may have severe security implications, since other websites will be able to read your clipboard, which may contain sensitive information such as passwords.</b>');
                        return;
                    }
            ...
            if (window.isSecureContext) {
                setInterval(check_clipboard, 1000);
            } else {
                clipboard_div.innerHTML = '<div class="error">Clipboard can not be monitored, because the page is not a <a href="https://developer.mozilla.org/en-US/docs/Web/Security/Secure_Contexts">secure context</a></div>'
            }
        } else {
            document.getElementById("clipboard-div").style.display = "none";
        }
    </script>
